...
anova_result = f_oneway(
    coverage_mean_to_list(coverage_sample1),
    coverage_mean_to_list(coverage_sample2),
    coverage_mean_to_list(coverage_sample3)
)
...
print(f"Total number of indels in sample 1: {indel_counter}") # (42)
print(f"Distance from first to last indel: {min_distance[0], min_distance[-1]}") # 1245282 to 1254357
...
counter = 0
above_mean_indel_counter = [
    value for sublist in indel_to_depth.values() for value in sublist
    if value > sample_1_depth["Depth"].mean()
]
counter = len(above_mean_indel_counter)

print(f"counter: {counter}") # 42 (so all indels are above the mean)
